...
-- 20260825120000_clear_out_16.sql, now keyed to the migrated business
-- admin account (atbatz@brain-spell.com) instead of the retired personal
-- tekdevbi@gmail.com -- clears all test teams/players/games/coaches/
-- members and every account except atbatz@brain-spell.com, keeps the
-- League/Division catalog intact.
do $$
declare
  v_keep_user uuid;
begin
  select id into v_keep_user from auth.users where email = 'atbatz@brain-spell.com';
  if v_keep_user is null then
    raise exception 'atbatz@brain-spell.com not found -- aborting reset';
  end if;
