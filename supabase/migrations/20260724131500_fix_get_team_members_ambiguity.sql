-- Fix "column reference user_id is ambiguous": RETURNS TABLE declares an
-- output column named user_id, which PL/pgSQL also exposes as an implicit
-- variable in scope -- the unqualified "select user_id from
-- coach_assignment / team_membership" inside the member-list subquery
-- collided with that variable, not just the table column. Qualifying with
