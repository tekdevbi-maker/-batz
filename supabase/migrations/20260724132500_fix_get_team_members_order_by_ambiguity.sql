-- Second half of the same bug: EVERY RETURNS TABLE output column name
-- (user_id, email, display_name, role, claimed_player_names) is also an
-- implicit PL/pgSQL variable, so ANY bare reference to one of those exact
-- names anywhere in the query body -- not just table columns, but a
-- SELECT-list alias too -- collides with the variable and raises "column
-- reference is ambiguous". The prior migration fixed the union subquery's
-- user_id; this fixes "order by ..., display_name", which hit the same
-- issue against the display_name alias. Using the ordinal position (3)
