-- Temporary debug RPC, dropped in the next migration once the "I'm the
-- Parent" showing on an already-claimed player bug is diagnosed.
...
returns table (
  ...
  season_status text,
  uniform_number int,
  parent_is_coach_on_team boolean
)
language sql
stable
security definer
...
as $$
  select
    ...
    p.parent_user_id, u.email::text, p.parent_attested_at,
    re.id, re.team_id, t.name, t.season_status, re.uniform_number,
    exists (select 1 from coach_assignment ca where ca.team_id = re.team_id and ca.user_id = p.parent_user_id)
  from player p
  ...
  where p.player_tag ilike '%' || p_query || '%'
