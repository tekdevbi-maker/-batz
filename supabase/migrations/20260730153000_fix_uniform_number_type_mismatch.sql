-- Fix: plpgsql's RETURN QUERY requires an exact type match against the
-- function's declared RETURNS TABLE columns (unlike a plain SQL-language
-- function, which is lenient) -- roster_entry.uniform_number is smallint,
-- but this was declared as plain int, throwing "structure of query does
-- not match function result type" every time a coach opened Team Members.
